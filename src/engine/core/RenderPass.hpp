 ...
 *
 * A SubPass is bound to a single graphics pipeline, which in itself is composed of a sequence of
 * shaders. A SubPass can be thought of as a single sequence of shaders executing on input data. For
 * a very simple rendering engine, we might only have a single SubPass that is executed and draws
 * the entire scene in a single pass.
 *
 * A more complex engine might have an initial SubPass that generates shadowmaps and writes them
 * into an attachment, followed by a second subpass that renders the scene using the shadowmaps as
 * well as the static / dynamic geometry. This is where SubPass dependencies come in: in this
 * example, the Shadowmap SubPass outputs are needed by the Rendering SubPass, which we might
 * describe as:
 * - The ShadowMap subpass (the first subpass in the RenderPass) depends on the VK_SUBPASS_EXTERNAL
 * subpass, to indicate that it needs to start after the previous set of frame commands has
 * finished.
 * - The ShadowMap subpass outputs into a depth attachment
 * - The Rendering subpass would depend on the previous subpass
 * - The Rendering subpass would depend on the previous src stage pipeline stage
 * VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT (which encompasses the depth attachment output)
 * - The Rendering subpass dst stage would be
 *
 * RenderPasses have a fixed resolution. All SubPasses within a RenderPass must have the same
 * resolution.
 */
class RenderPass {
 public:
...
class Subpass {
 public:
   ...
   */
  Subpass(const RenderPass& parent,
          uint32_t index,
          std::vector<std::reference_wrapper<const Attachment>> colorAttachments);

  const std::vector<VkAttachmentReference>& getColorAttachmentReferences() const {
    return colorAttachmentReferences_;
  }
 ...
 *
 * When creating a RenderPass, we fill it with SubPasses. Each subpass consumes a number of
 * attachments, which are the buffers that pass between SubPasses within a render pass, or are
 * buffers supplied to the Shader, or buffers emitted by the Shader to the display system.
 *
 * Attachments are later described as part of a VkFramebuffer object. There needs to be a 1:1
 * mapping between Attachments provided to the subpass and the VkFramebuffer that gets bound to the
 * pipeline.
 *
 * It seems like the idea is that we can generate multiple compatible VkFramebuffer objects and
 * round-robbin swap them as we cycle through frames. This allows us to have multiple frames in
 * flight.
 *
